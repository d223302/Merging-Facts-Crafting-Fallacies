...
class Retrieval(object):
    ...
    def get_passages(self, topics, name, question, k):
        """
        topics: a list of topic strings, which are the titles of the retrieved passages
        name: the name of the person (remove the disambiguation information from topic)
        question: the question string
        k: the number of passages to retrieve
        Return: a list of lists of retrieved passages, where each sublist corresponds to a topic (a page in Wikipedia)
        """
        if type(topics)==str:
            topics = [topics]
        retrieved_passages = [[] for _ in range(len(topics))]

        for topic_idx, topic in enumerate(topics):
            retrieval_query = name + " " + question.strip()
            cache_key = topic + "#" + name + "#" + retrieval_query

            if cache_key not in self.cache:
                passages = self.db.get_text_from_title(topic)
                if self.retrieval_type=="bm25":
                    single_retrieved_results = self.get_bm25_passages(topic, retrieval_query, passages, k)
                    self.cache[cache_key] = single_retrieved_results
                    retrieved_passages[topic_idx].extend(single_retrieved_results)
                else:
                    single_retrieved_results = self.get_gtr_passages(topic, retrieval_query, passages, k)
                    self.cache[cache_key] = single_retrieved_results
                    retrieved_passages[topic_idx].extend(single_retrieved_results)
                assert len(self.cache[cache_key]) in [k, len(passages)]
                self.add_n += 1
            else:
                retrieved_passages[topic_idx].extend(self.cache[cache_key])
