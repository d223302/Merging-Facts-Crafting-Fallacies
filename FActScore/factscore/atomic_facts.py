...
class AtomicFactGenerator(object):
    ...
    def get_atomic_facts_from_paragraph(self, paragraphs, cost_estimate=None):
            ...
            [sent for i, sent in enumerate(sentences) 
             if not (
                 not self.is_bio and (
                     ...
                     (i == len(sentences) - 1 and (sent.startswith("Please") or sent.startswith("I hope") or sent.startswith("Here are"))) or
                     (sent.startswith("There are") and "individuals" in sent) or sent.startwith("Further information") or "further information" in sent
                 )
             )
            ], 
        ...
        for i, sent in enumerate(sentences):
            if not self.is_bio and ( \
                ...
                atomic_facts_pairs.append((sent, []))
            elif (sent.startswith("There are") and "individuals" in sent) or sent.startswith("Further information") \
                or "further information" in sent:
                atomic_facts_pairs.append((sent, []))
            else:
    ...
    def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
        ...
        for sentence in sentences:
            ...
            for i in range(n):
                prompt = prompt + "Please breakdown the following sentence into independent facts. When possible, decontextualize the fact such that the subject in the fact is a proper noun or noun instead of a pronoun: {}\n".format(list(demons.keys())[i])
