...
class LLM:
    ...
    def __init__(self, args):
        ...
        elif args.claude_api:
            import anthropic
            from anthropic import HUMAN_PROMPT, AI_PROMPT
            self.claude_api_key = os.environ.get("CLAUDE_API_KEY")
            
            # Since I cannot find the tokenizer for Claude, I use the tokenizer for GPT2
            self.tokenizer = AutoTokenizer.from_pretrained("gpt2", fast_tokenizer=False) # TODO: For ChatGPT we should use a different one
            # To keep track of how much the API costs
            self.prompt_tokens = 0
            self.completion_tokens = 0

        elif args.gemini_api:
            import google.generativeai as genai
            self.gemini_api_key = os.environ.get("GEMINI_API_KEY")
            self.tokenizer = AutoTokenizer.from_pretrained("t5", fast_tokenizer=False)
            self.prompt_tokens = 0
            self.completion_tokens = 0
    ...
    def generate(self, prompt, max_tokens, stop=None):
        ...
        if args.openai_api:
            ...
            import anthropic
            from anthropic import HUMAN_PROMPT, AI_PROMPT
            message = f"{HUMAN_PROMPT} You are a helpful assistant that answers the following questions with proper citations.\n\n{prompt}{AI_PROMPT}"
            print(message)
            ...
            while not is_ok:
                ...
                if "type" in response and response["type"] == "error":
                    if retry_count <= 5:
                        logger.warning(f"Claude API retry for {retry_count} times ({response})")
                        continue
                    print(response)
                    import pdb; pdb.set_trace()
                else:
            ...
            message = f"You are a helpful assistant that answers the following questions with proper citations.\n\n{prompt}"
            
            @retry.Retry()
            def call_google_genai(message, model):
                response = model.generate_content(message)
                return response
            
            response = call_google_genai(message, model)
...
def main():
    ...
    parser.add_argument("--claude_api", action="store_true", default=False, help="Whether to use Claude API")
    parser.add_argument("--gemini_api", action="store_true", default=False, help="Whether to use Gemini API")
    ...
    if "16k" in args.model:
        ...
        args.max_length = 4096
    elif "tulu" in args.model.lower():
        args.max_length = 8192
    elif "vicuna" in args.model.lower():
        args.max_length = 4096
    elif "claude" in args.model.lower():
        args.max_length = 100_000
